    ...
    def sample(self, category, start_letter="A"):
        category_tensor = Variable(
            self.gen_category_tensor(
                self.all_categories, self.n_categories, category
            )
        )
        input_tensor = Variable(
            self.gen_input_tensor(
                self.all_letters, self.n_letters, start_letter
            )
        )
        ...
        for i in range(max_length):
            ...
            input_tensor = Variable(
                self.gen_input_tensor(self.all_letters, self.n_letters, letter)
            )
